...
export const askGenesisAdvisor = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Fetch macro, knowledge, real FRED forecast, and sector ETF data in parallel
    const [macro, knowledge, realForecast, realSectors] = await Promise.all([
    ...
    const userMessage = [
      ...
      realForecast ? `${realForecast.overallForecastSummary}\n\n` : "",
      realSectors  ? `${realSectors.arabicAnalysis}\n\n`          : "",
