...
async function aiSentimentBatch(
  ...
  const prompt = `حلل معنويات السوق الحالية لكل أصل من القائمة التالية بناءً على المناخ الاقتصادي والجيوسياسي العام والأخبار الحديثة. لكل أصل أعطِ:
- sentiment: درجة من -100 (سلبي جداً) إلى +100 (إيجابي جداً)
- note: سبب مختصر بالعربية (جملة واحدة)

الأصول:
${list}

أعد JSON فقط بصيغة: { "RESULTS": [ { "symbol": "...", "sentiment": -50..100, "note": "..." } ] }`;
